...
TEST_CASE("nano-unicode-sv", unicode_string_view) {
  {
    EXPECT_EQ(utf::string_view(getTestString()).count(), utf::string_view(getTestWString()).count());
    EXPECT_EQ(utf::string_view(getTestString()).count(), utf::string_view(getTestWString()).to_utf32().size());
  }

  {
    std::wstring a = utf::to_wide(utf::string_view(getTestString()));
    std::wstring aa = utf::to_wide(getTestString());
    std::wstring b = getTestWString();
    EXPECT_EQ(a.size(), b.size());
    EXPECT_EQ(aa.size(), b.size());
  }

  {
    std::string a = utf::to_utf8(getTestString());
    std::string b = utf::to_utf8(getTestWString());
    EXPECT_EQ(a.size(), b.size());
  }

  {
    utf::string_view a(getTestString());
    ...
    EXPECT_TRUE(a.u16str().empty());
    EXPECT_TRUE(a.u32str().empty());
    EXPECT_TRUE(a.wstr().empty());
  ...
  {
    utf::string_view a(getTestWString());

    EXPECT_EQ(a.u8cstr(), nullptr);
    EXPECT_NE(a.wcstr(), nullptr);

    EXPECT_TRUE(a.u8str().empty());
